...
def runBioProspector():
    ...
    with open("results/bioprospector.out", "w") as fout:
        fasta2bp.convert(POS_SEQ, "bp_"+get_filename(POS_SEQ))
        cmdline = "./BioProspector/BioProspector -i "+"bp_"+get_filename(POS_SEQ) \
            + " -o " + RES_DIR + "bp_output.txt -W "+str(MOTIF_LEN)
        subprocess.call([cmdline], stdout=fout, stderr=fout, shell=True)
...
def parseWeeder(fout):
    foundMotifs["Weeder"] = []
    pass
    # with open(RES_DIR + get_filename(POS_SEQ) + ".w2", "r") as _weederResults:
    #     foundMotifs["Weeder"] = []
...
def main():
    ...
    runTools()
    parseResults()
    print "Time taken per tool (may be overestimate)"
    for tool in times:
        print tool + " " + str(int(times[tool]["End"]) - int(times[tool]["Start"]))
    print foundMotifs
    combineMotifs()
    visualizeOutput(RES_DIR, mlist, POS_SEQ, POS_SEQ[POS_SEQ.rfind('/')+1:POS_SEQ.rfind('.')]) #kinda hacky
